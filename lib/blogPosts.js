const { slugify, isPlaceholderSlug } = require('./slugify');
...
/** Old slugify stripped Bengali matras → "ক-ন-জ-প…" style broken slugs. */
function legacyMatraStrippedSlug(text) {
  return String(text || '')
    .normalize('NFC')
    .trim()
    .toLowerCase()
...
/** Fix posts saved as /blog/item or matra-stripped Bangla slugs. */
async function repairPlaceholderBlogSlugs(queryFn) {
  let rows = [];
  try {
    rows = await queryFn(`SELECT id, title, slug FROM blog_posts`);
  } catch (_) {
    return { repaired: 0 };
  }
  let repaired = 0;
  for (const row of rows || []) {
    ...
    const brokenLegacy = legacyMatraStrippedSlug(title);
    const needsRepair =
      isPlaceholderSlug(current) ||
      (brokenLegacy && current === brokenLegacy && desired && desired !== current);
    if (!needsRepair) continue;
    const next = await ensureUniqueBlogSlug(queryFn, title, row.id);
    if (!next || next === current) continue;
    await queryFn('UPDATE blog_posts SET slug = ? WHERE id = ?', [next, row.id]);
    repaired += 1;
  }
  return { repaired };
}
