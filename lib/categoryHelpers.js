const { normalizeStoreUrl } = require('./normalizeStoreUrl');
...
function normalizeCategoryRow(row) {
  ...
  return {
    ...
    parent_id: normalizeCategoryId(row.parent_id ?? row.parentId),
    slug: normalizeStoreUrl(row.slug),
    parent_slug: row.parent_slug ? normalizeStoreUrl(row.parent_slug) : row.parent_slug,
...
async function resolveCategoryIdsBySlug(query, slug) {
  ...
  if (!s || s === 'all' || s.startsWith('/') || /^https?:\/\//i.test(s)) return [];
