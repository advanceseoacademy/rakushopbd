...
const DEFAULT_HELP = JSON.stringify([
  ...
  { label: 'Reseller', href: 'https://reseller.rakushopbd.com' },
]);
...
const DEFAULTS = [
  ...
  ['site_logo_url', '/images/rakushopbd-logo.png?v=14'],
...
async function migrateSiteLogoUrl() {
  const rows = await query(
    'SELECT setting_value FROM site_settings WHERE setting_key = ? LIMIT 1',
    ['site_logo_url']
  );
  if (!rows.length) return;
  const current = String(rows[0].setting_value || '').trim();
  if (!current || current === DEFAULT_LOGO_URL) return;
  const isBundled =
    current === DEFAULT_LOGO_PATH || /^\/images\/rakushopbd-logo\.png\?v=\d+$/.test(current);
  const isLegacyUpload = /\/uploads\//i.test(current) && /(logo|rakushop|brand)/i.test(current);
  if (isBundled || isLegacyUpload) {
    await query(upsertSiteSettingSql(), ['site_logo_url', DEFAULT_LOGO_URL]);
  }
}

async function migrateFooterRemoveRewardsLink() {
  const keys = ['footer_help_links', 'footer_quick_links'];
  for (const key of keys) {
    ...
    const next = links.filter((link) => {
      const normalized = normalizeHelpLink(link);
      if (!normalized) return false;
      if (normalized.page === 'rewards') return false;
      const href = String(normalized.href || '').trim();
      if (href === '/rewards') return false;
      const label = String(normalized.label || '').trim().toLowerCase();
      return label !== 'raku rewards';
    });
    if (JSON.stringify(next) === JSON.stringify(links)) continue;
    await query(upsertSiteSettingSql(), [key, JSON.stringify(next)]);
  }
}

async function migrateFooterAddAboutLink() {
  const rows = await query(
    'SELECT setting_value FROM site_settings WHERE setting_key = ? LIMIT 1',
    ['footer_quick_links']
  );
  ...
  const hasAbout = links.some((link) => {
    const normalized = normalizeHelpLink(link);
    if (!normalized) return false;
    if (normalized.page === 'about') return true;
    const href = String(normalized.href || '').trim();
    if (href === '/about' || href === '/about-us') return true;
    return String(normalized.label || '').trim().toLowerCase() === 'about us';
  });
  if (hasAbout) return;
  const next = dedupeFooterLinks([{ label: 'About Us', href: '/about' }, ...links.map(normalizeHelpLink)]);
  await query(upsertSiteSettingSql(), ['footer_quick_links', JSON.stringify(next)]);
}

async function migrateFooterAddResellerLink() {
  const rows = await query(
    'SELECT setting_value FROM site_settings WHERE setting_key = ? LIMIT 1',
    ['footer_help_links']
  );
  ...
  if (!Array.isArray(links)) return;
  const hasReseller = links.some((link) => {
    const href = String(link?.href || '').trim().toLowerCase();
    const label = String(link?.label || '').trim().toLowerCase();
    return href.includes('reseller.rakushopbd.com') || label === 'reseller';
  });
  if (hasReseller) return;
  const next = dedupeFooterLinks([
    ...links.map(normalizeHelpLink),
    { label: 'Reseller', href: 'https://reseller.rakushopbd.com' },
  ]);
  await query(upsertSiteSettingSql(), ['footer_help_links', JSON.stringify(next)]);
}
...
async function ensureFooterSettings() {
  ...
  try {
    ...
    await migrateSiteLogoUrl();
    await migrateFooterRemoveRewardsLink();
    await migrateFooterHelpLinks();
    await migrateFooterQuickLinks();
    await migrateFooterAddAboutLink();
    await migrateFooterAddResellerLink();
