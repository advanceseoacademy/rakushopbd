    ...
    if (existing.length) {
      await query(
        `UPDATE product_reviews
         SET product_id = ?, customer_name = ?, rating = ?, comment = ?, reviewer_city = ?,
             homepage_sort_order = ?, status = 'approved'
         WHERE homepage_seed_key = ?`,
        [productId, profile.name, profile.rating, comment, profile.city || null, i, seedKey]
      ).catch(() => {});
      productIdsTouched.add(Number(existing[0].product_id));
