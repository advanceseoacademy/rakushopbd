...
function variantPublicPath(uploadPath, width) {
  ...
  if (!u.startsWith('/uploads/')) return u;
  const w = Math.min(MAX_VARIANT_WIDTH, Math.max(48, Math.round(Number(width) || 0)));
  if (!w) return u;
  const rel = u.replace(/^\//, '');
  return `/media/${w}/${rel}`;
}
...
async function getOrCreateVariant(absSource, width) {
  ...
  let pipeline = sharp(absSource, { failOn: 'none' }).rotate();
  if (meta.width && meta.width > targetW) {
    pipeline = pipeline.resize({ width: targetW, withoutEnlargement: true });
  }
  await pipeline.webp({ quality: VARIANT_QUALITY, effort: 4 }).toFile(cachePath);
