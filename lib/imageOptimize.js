...
async function optimizeBufferToWebp(buffer, maxWidth = MAX_WIDTH) {
 ...
 */
async function optimizeAndSaveImage(file, opts = {}) {
  if (!file?.buffer?.length) {
    throw new Error('Empty image file');
  }

  const maxWidth = Number(opts.maxWidth) || MAX_WIDTH;
  const bytesBefore = file.buffer.length;
  const filename = `${Date.now()}-${safeBaseName(file.originalname)}.webp`;
  const outPath = path.join(uploadDir, filename);

  const output = await optimizeBufferToWebp(file.buffer, maxWidth);
