...
const { requireAdminOrLocal } = require('./requestGuards');
const { createRateLimiter } = require('./simpleRateLimit');

const rateLimitAdminLogin = createRateLimiter({
  windowMs: 15 * 60 * 1000,
  max: 12,
...
function registerAdminAuthRoutes(register) {
  register('get', '/version', requireAdminOrLocal, (req, res) => {
    res.json({ ok: true, apiVersion: 2, hasAuthToken: true, source: register.source });
  });

  register('get', '/ping', requireAdminOrLocal, async (req, res) => {
    try {
        ...
        hint:
          err.code === 'MODULE_NOT_FOUND'
            ...
            : !process.env.DATABASE_URL
              ...
              : 'git pull + STOP → START; test /api/db-check on the server',
      });
    }
  });

  register('post', '/login', rateLimitAdminLogin, async (req, res) => {
    try {
...
function registerAdminAuth(app) {
  const register = (method, subpath, ...handlers) => {
    app[method](`/api/admin${subpath}`, ...handlers);
  };
...
function registerAdminAuthApiRouter(router) {
  const register = (method, subpath, ...handlers) => {
    router[method](`/admin${subpath}`, ...handlers);
  };
