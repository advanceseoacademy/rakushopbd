...
function mapVideoRow(row) {
  ...
  return {
    ...
    viewedByAdmin:
      row.viewed_by_admin === true ||
      row.viewed_by_admin === 1 ||
      row.viewed_by_admin === '1' ||
      row.viewed_by_admin === 't' ||
      row.viewed_by_admin === 'true',
    ...
    await query(
      `UPDATE product_review_videos
       SET video_url = ?, status = 'pending', admin_note = '', reviewed_at = NULL, created_at = CURRENT_TIMESTAMP,
           viewed_by_admin = false, viewed_at = NULL
...
async function countPendingReviewVideos(query) {
  const rows = await query(
    `SELECT COUNT(*) AS c FROM product_review_videos WHERE status = 'pending'`
  );
  if (!rows?.[0]) return 0;
  return Number(Object.values(rows[0])[0]) || 0;
}

async function countUnreadReviewVideos(query) {
  const rows = await query(
    `SELECT COUNT(*) AS c FROM product_review_videos WHERE viewed_by_admin IS NOT TRUE`
  );
  if (!rows?.[0]) return 0;
  return Number(Object.values(rows[0])[0]) || 0;
}
