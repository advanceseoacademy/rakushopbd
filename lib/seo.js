...
function productSeoDescription(product, settings) {
  ...
  return product.description_bn || settings.seo_meta_description || '';
}
...
async function getCategoryBySlug(slug) {
  ...
  const rows = await query(
    `SELECT id, slug, name_bn, name_en, icon FROM categories WHERE slug = ? LIMIT 1`,
    [slug]
  );
  return rows[0] || null;
}
...
async function buildPageSeo(req, { bootstrap, product, category } = {}) {
  ...
  if (NOINDEX_PAGES.has(pageType)) {
    ...
    description = truncate(
      seoDesc || product.description_bn || `${name}. Order from ${siteName} with fast delivery in Bangladesh.`
    );
    ...
    jsonLd.push(
      buildBreadcrumbJsonLd(base, [
        { name: 'Home', url: '/' },
        { name, url: `/category/${category.slug}` },
      ])
    );
