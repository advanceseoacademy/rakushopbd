...
async function getStoreBootstrap(req) {
  ...
  const [
    ...
    getHomeProductSections(query, 24),
    query(
      `SELECT
  ...
  const s = statsRow[0] || {};
  let { bestSelling, newArrivals } = homeSections;
  const productCount = Number(s.product_count ?? s.productcount) || 0;

  // Retry once if parallel startup queries returned empty but products exist (VPS cold start).
  if (productCount > 0 && !bestSelling.length && !newArrivals.length) {
    clearHomeProductsCache();
    await new Promise((r) => setTimeout(r, 1500));
    const retry = await getHomeProductSections(query, 24);
    bestSelling = retry.bestSelling;
    newArrivals = retry.newArrivals;
  }
  ...
  const hasHomeProducts = payload.bestSelling.length || payload.newArrivals.length;
  if (hasHomeProducts || productCount === 0) {
    cache = payload;
    cacheAt = Date.now();
  }
  return payload;
