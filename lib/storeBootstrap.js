...
const { getTodaySellingProducts, getTodaySellingMeta } = require('./todaySelling');
...
async function getStoreBootstrap(req, opts = {}) {
  ...
  let todaySelling = [];
  try {
    todaySelling = await getTodaySellingProducts(query, settings);
  } catch (err) {
    console.warn('today selling products:', err.message);
  }

  const todaySellingMeta = getTodaySellingMeta(settings);
  ...
  const payload = {
    ...
    todaySelling: stripInternalProductList(todaySelling),
    todaySellingMeta,
    stats: {
