...
const { parseRewardsContent } = require('./rewardsPage');
...
async function getStoreBootstrap(req, opts = {}) {
  ...
  if (!lite && cache && Date.now() - cacheAt < TTL_MS && !adminPreview && cacheIsUsable(cache)) {
    return cache;
  }
  ...
  const settings = withMarketingDefaults({ ...settingsRaw });
  settings.rewards_page_content = JSON.stringify(
    parseRewardsContent({ rewards_page_content: settingsRaw.rewards_page_content })
  );
  ...
  const heroSideSlider = getHeroSideSlider(settings);

  let todayDeals = [];
  try {
    todayDeals = await getTodayDealsProducts(query, settings);
  } catch (err) {
    console.warn('today deals products:', err.message);
  }
  const todayDealsMeta = getTodayDealsMeta(settings);
  ...
  const payload = {
    ...
    heroSideSlider,
    todayDeals: stripInternalProductList(todayDeals),
    todayDealsMeta,
    stats: {
