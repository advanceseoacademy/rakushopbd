    ...
    thumbRow.innerHTML = urls
      .map(
        (url, i) => {
          ...
          return `<div class="thumb-img${i === 0 ? ' active' : ''}" data-url="${encodeURIComponent(productImageSrc(url))}" style="background:${p.bg_color || '#f5f5f5'};"><img src="${src}" alt="" loading="lazy"></div>`;
        }
      )
      .join('');
